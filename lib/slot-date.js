...
export function summarizeSlotDiagnostics(rows) {
  ...
  let checksWithUnrecognizedDateButtons = 0;
  ...
  for (const check of checks) {
    ...
    const dateButtons = check?.slotDebug?.dateButtons || [];
    if (
      reason === "target-date-not-found" &&
      dateButtons.length > 0 &&
      dateButtons.some(label => !parseSlotDateText(label))
    ) {
      checksWithUnrecognizedDateButtons += 1;
    }
  }
  ...
  return {
    ...
    checksWithUnrecognizedDateButtons,
    scheduleFormatMismatch: checks.length > 0 &&
      checksWithRenderedTimes > 0 &&
      unrecognizedChecks === checks.length &&
